          ...
          <div className="space-y-3">
            {baskets.map((basket) => (
              ...
              >
                  ...
                  <div 
                    className={`w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0 transition-colors border ${
                      activeBasketId === basket.id 
                        ...
                        : 'bg-muted/40 text-foreground/50 border-border/40'
                    }`}
                  >
                ...
                {/* Three-dot Menu - Redesigned with Popover */}
                <div className={`absolute left-2 top-1/2 -translate-y-1/2 transition-opacity ${
                  ...
                  >
                    <PopoverTrigger asChild>
                      <button 
                        className="p-2 rounded-lg hover:bg-muted/60 transition-colors border border-transparent hover:border-border/40"
                    ...
                    <PopoverContent 
                      align="start" 
                      side="left"
                      className="w-52 p-2 border border-border/60 shadow-lg"
                      onClick={(e) => e.stopPropagation()}
                    >
                      {!showMergeOptions ? (
                        <div className="space-y-1">
                          ...
                          {baskets.length > 1 && (
                            <button
                              ...
                              className="w-full flex items-center justify-between gap-2.5 px-3 py-2.5 rounded-lg text-sm text-foreground hover:bg-muted/60 transition-colors"
                            >
                        ...
                        <div className="space-y-1">
                          <button
                            ...
                            className="w-full flex items-center gap-2 px-3 py-2 rounded-lg text-xs text-muted-foreground hover:bg-muted/40 transition-colors mb-1"
                          >
                            <ChevronDown className="w-3 h-3 rotate-90" />
                            بازگشت
                          </button>
                          <div className="text-xs text-muted-foreground px-3 py-1.5 border-b border-border/40 mb-1">
                            ادغام با:
                          </div>
        ...
        <div className="px-4 py-3 border-t border-border/30">
          ...
          {expandedSections['saved-items'] && (
            <div className="mt-3 space-y-2">
              {activeSavedItems.length === 0 ? (
                <p className="text-xs text-muted-foreground text-center py-4 bg-muted/20 rounded-xl border border-border/30">
                  محصولی ذخیره نشده
                </p>
              ) : (
                activeSavedItems.map((item) => (
                  <div 
                    key={item.id}
                    className="flex items-center gap-3 p-3 rounded-xl bg-background border border-border/40 group hover:border-border/60 transition-colors"
                  >
                    <img 
                      src={item.image} 
                      alt={item.name}
