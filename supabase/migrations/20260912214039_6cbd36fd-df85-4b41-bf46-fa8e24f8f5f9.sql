DROP FUNCTION IF EXISTS public.pet_question_facets(text, text, text, text, text, text, text, boolean, text[], text, text[], text[], boolean);

CREATE OR REPLACE FUNCTION public.pet_question_facets(
  p_query text DEFAULT NULL::text, p_category text DEFAULT NULL::text, p_subcategory text DEFAULT NULL::text,
  p_subcategory_prefix text DEFAULT NULL::text, p_species text DEFAULT NULL::text, p_brand text DEFAULT NULL::text,
  p_origin_country text DEFAULT NULL::text, p_in_stock boolean DEFAULT true, p_needs text[] DEFAULT NULL::text[],
  p_type_group text DEFAULT NULL::text, p_product_types text[] DEFAULT NULL::text[], p_exclude_brands text[] DEFAULT NULL::text[],
  p_foreign_only boolean DEFAULT NULL::boolean, p_life_stage text DEFAULT NULL::text)
 RETURNS jsonb
 LANGUAGE sql
 STABLE
...
base AS (
  ...
  WHERE (p_in_stock IS NOT TRUE OR p.in_stock)
    ...
    -- Life stage: exact match or unknown (three-valued); rows labelled for another stage are excluded.
...
GRANT EXECUTE ON FUNCTION public.pet_question_facets(text, text, text, text, text, text, text, boolean, text[], text, text[], text[], boolean, text) TO anon, authenticated, service_role;